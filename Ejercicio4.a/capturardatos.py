...
def funcion(time, rdd):      
    ...
    if rdd.count() > 0:
        ...
        df.createOrReplaceTempView("screen_name")
        df = spark.sql("select name, count(*) as total, timestamp from screen_name group by name, timestamp order by total desc limit 5")
        df.write.mode("append").saveAsTable("tb_screenname")
        ...
        if data.count()>0:
            ...
            df.show()
            df.write.mode("append").saveAsTable("tb_ocurrences")
        else:
            print("rdd is empty: no action")
        
if __name__ == "__main__":
    sc = SparkContext(appName="Project BigData 2: operations")
    readtweets() 
